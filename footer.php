    ...
    <nav id="share-menu" class="close">
        <ul>
            ...
            <li class="evernote"><a href="#" onclick="Evernote.doClip({url:'<?php echo get_meta_url();?>',providerName:'<?php bloginfo('name');?>',title:'<?php the_title();?>',contentId:'site-main',}); return false;"><i class="fa fa-sticky-note-o fa-5x" aria-hidden="true"></i></a></li>
    ...
    <script>
        function(){document.body.addEventListener("click",drop,false);function drop(e){var x = e.pageX;var y = e.pageY;var sizuku = document.createElement("div");sizuku.style.top = y + "px";sizuku.style.left = x + "px";document.body.appendChild(sizuku);sizuku.className = "sizuku";sizuku.addEventListener("animationend",function(){this.parentNode.removeChild(this);},false);};};
        function(){var class=document.getElementById("menu-toggle");class.onclick(function(){class.classList.toggle('close');class.classList.toggle('open');});};
