<style>
	...
	*/
	...
	.card-eyecatch{
		...
		float:left;
		vertical-align:middle;
		width:calc(72vmin / 2 - 1vmin);
	}
	.card-info{
		float:right;
		max-width:calc(72vmin / 2);
	}
	.card-title{
		display:inline-block;
		height:calc(40.5vmin / 10 * 3);
		margin:0 1em;
	}
	.card-meta{
		display:inline-block;
		height:calc(40.5vmin / 10 * 7);
		margin:0 1em;
	}
	.site-info{
		text-align:center;
	...
	<?php if(is_singular()===true):
		...
		11.codex
		*/
		$meta_background = color_to_rgb($colorcode = get_option('article_meta_background','#f1f1f1'));
		?>
		.article-header{
			height:0;
			padding-top:85%;
		...
		.article-meta{
			background-image:linear-gradient(to top,<?php echo'rgba(' . $meta_background["red"] . ',' . $meta_background["green"] . ',' . $meta_background["blue"] . ',.8)';?>,rgba(0,0,0,.15));
			color:<?php echo get_option('article_meta_color','#fff');?>;
			font-size:2rem;
			min-height:15vh;
			width:100%;
		}
